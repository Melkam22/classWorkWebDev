...
/* Map, filter, reduce works only with array & similar to
...
function mapInAction (mapped){
    let myMap = mapped.map(action=>action**2)
    console.log(myMap);
}
mapInAction([1,2,3,4]);
 ...
 /* or same filter */
function checkAges1(age1){
    let filterIt = age1.filter(function(value){
        return value >=18;
    });
    console.log(filterIt);
 }
 checkAges1([17,32,44,11,7,57,85,99]);//we ll get[32,44,57,85,99] too
...
function myFilter2 (result){
let filterIt1 = result.filter(function(words){
    return words.length <= 4;
})
console.log(filterIt1);
}
myFilter2(["spray", "Ashu", "spread", "melkam", "come"]);//we ll get 'ashu', 'come'] too
...
let givenArray4= [1,2,3,4,5];
let myAdder = (current, accumule)=>
    current + accumule;
let myReduce = givenArray4.reduce(myAdder, 10);
console.log(myReduce);//we ll get 25 (givenArray + 10) too
 ...
 for(let a=0; a<months.length; a++){
    let firstLetter = months[a].substring(0,1).toUpperCase();
    let totalLetter = months[a].substring(1,Infinity);
    let fullWord = firstLetter+totalLetter;
    let myResult = fullWord.sort();
    console.log(myResult);//why sort()is not working??
   
 }
  
 
 console.log("_________________ sort ex 1 _____________________");
