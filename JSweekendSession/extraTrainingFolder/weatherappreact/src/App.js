...
class App extends Component {
  ...
  componentDidMount() {
    let myApi = `http://api.openweathermap.org/data/2.5/weather?q=berlin&APPID=${Home}`;
    fetch(myApi)
  ...
  clickSubmit(e) {
    ...
    let myApi = `http://api.openweathermap.org/data/2.5/weather?q=${this.state.input}&APPID=${Home}`
    fetch(myApi)
      .then(response => response.json())
      .then(data => {
        this.setState({
      ...
      <div className="main">
        ...
        <div /* clsNm container added above */ className={(this.state.description)}>
          <div className="title">
            ...
            <span className="span">
              <img src={`http://openweathermap.org/img/wn/${this.state.icon}.png`} alt="weather img" />
            </span>
          </div>
